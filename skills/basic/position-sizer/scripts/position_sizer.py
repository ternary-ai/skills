...
Usage:
    # Fixed Fractional
    python3 position_sizer.py --account-size 100000 --entry 155 --stop 148.50 --risk-pct 1.0

    # ATR-Based
    python3 position_sizer.py --account-size 100000 --entry 155 --atr 3.20 --atr-multiplier 2.0 --risk-pct 1.0

    # Kelly budget mode
    python3 position_sizer.py --account-size 100000 --win-rate 0.55 --avg-win 2.5 --avg-loss 1.0

    # Kelly shares mode
    python3 position_sizer.py --account-size 100000 --entry 155 --stop 148.50 --win-rate 0.55 --avg-win 2.5 --avg-loss 1.0
"""
...
def calculate_kelly(params: SizingParameters) -> dict:
    ...
    kelly_pct = w - (1 - w) / r
    kelly_pct = max(0.0, kelly_pct) * 100  # convert to %, floor at 0
...
def apply_constraints(
    shares: int, params: SizingParameters
) -> tuple[int, list[dict], str | None]:
    """Apply portfolio constraints. Returns (final_shares, constraints, binding_name).

    Strictest constraint (minimum share count) wins.
    """
...
def calculate_position(params: SizingParameters) -> dict:
    """Main entry point. Determines mode, runs sizing, applies constraints."""
    validate_parameters(params)

    is_kelly_mode = params.win_rate is not None
    has_entry = params.entry_price is not None

    result: dict = {"schema_version": "1.0", "parameters": {}}
...
def generate_markdown_report(result: dict) -> str:
    ...
    if result["mode"] == "budget":
        ...
        lines.append(
            "- **Recommended Risk Budget:** ${:,.2f}".format(
                result["recommended_risk_budget"]
            )
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--avg-win", type=float,
                        help="Average win amount for Kelly criterion")
    parser.add_argument("--avg-loss", type=float,
    ...
    parser.add_argument("--max-sector-pct", type=float,
                        help="Maximum sector exposure as %% of account")
    parser.add_argument("--sector", type=str,
                        help="Sector name for concentration check")
    parser.add_argument("--current-sector-exposure", type=float, default=0.0,
...
def main() -> None:
    ...
    if args.risk_pct is not None and args.win_rate is not None:
        parser.error("specify either --risk-pct (Fixed/ATR mode) OR --win-rate (Kelly mode), not both")
    ...
    if args.win_rate is not None:
        ...
        parser.error("Must specify either --risk-pct (Fixed/ATR) or --win-rate (Kelly)")
